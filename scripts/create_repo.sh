...
HADOOPBAM_VERSIONS=( "6.0" "6.1" "6.2" )
SNAPSHOT_VERSION="6.3-SNAPSHOT"
SNAPSHOT_MAVEN_VERSION="6.3-SNAPSHOT"
...
for version in ${HADOOPBAM_VERSIONS[@]}; do
	...
	$(cd $TMPDIR && \
        ...
        fetch_and_build $version hadoop-bam org.seqdoop.hadoopbam hadoop-bam Hadoop-BAM
done
...
mvn deploy:deploy-file -Durl=$OUTPUT_URL -DrepositoryId=$REPO_ID -Dfile=$TMPDIR/hadoop-bam-${SNAPSHOT_VERSION}/target/hadoop-bam-${SNAPSHOT_MAVEN_VERSION}.jar \
    -DgroupId=org.seqdoop.hadoopbam -DartifactId=hadoop-bam -Dversion="$SNAPSHOT_VERSION" -DgeneratePom.description="Hadoop-BAM"
